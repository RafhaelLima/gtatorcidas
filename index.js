...
client.once('ready', () => {
	...
	client.user.setActivity('GTA Torcidas - Feliz Natal', { type: 'PLAYING' })
});
...
client.on('message', message => {
	...
	if (!command) return;
	if (command.guildOnly && message.channel.type !== 'text') return message.reply('E aí chefe! Por que está sussurando aqui no privado? * Risos * - Enfim, não posso executar nenhum comando no privado, apenas nos canais de bot!');
	...
	if (timestamps.has(message.author.id)) {
		...
		if (now < expirationTime) {
			...
			return message.reply(`Opa, vai com calma! Espere ${timeLeft.toFixed(1)} segundos para usar o comando \`${command.name}\``);
		}
	}
